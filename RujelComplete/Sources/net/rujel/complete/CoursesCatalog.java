...
public class CoursesCatalog extends com.webobjects.appserver.WOComponent {
    ...
    public static void prepareCourses(File folder, WOContext ctx) {
		...
		while (enu.hasMoreElements()) {
			...
			if(writeReports)
				printCourseReports(course, folder, ctx, reports, false);
		}*/
    }
    ...
    public static void printCourseReports(EduCourse course, File cDir, WOContext ctx,
		...
		EOKeyGlobalID gid = (EOKeyGlobalID)ec.globalIDForObject(course);
		cDir = new File(cDir,gid.keyValues()[0].toString());
		if(!cDir.exists())
			cDir.mkdirs();
...
//		page = WOApplication.application().pageWithName("PrintLessons", ctx);
//    	page.takeValueForKey(course, "course");
//		Executor.writeFile(cDir, "lessons.html", page,overwrite);
    	if(reports == null || reports.count() == 0)
    		return;
    	NSMutableDictionary ready = CoursePage.readyModules(course, reports);
    	...
    	while (enu.hasMoreElements()) {
			...
			if(ready.valueForKey(id) == Boolean.FALSE) {
				if(overwrite) {
					if(file.exists())
						file.delete();
				}
...
//			page.takeValueForKey(rep,"reporter");
			page.takeValueForKey(course,"course");
			Executor.writeFile(file, page,overwrite);
		}
    }
}